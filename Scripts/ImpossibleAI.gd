...
func defense():
	...
	if(grounded):
		...
		if(ball != null):
			var angle
			if up_direction.y < 0:
				angle = int(rad_to_deg((up_direction * 100).angle_to(%Player.global_position - global_position)))
				#print("on the ground")
				if angle > 0:
					left = 1
				else:
				...
				angle = int(rad_to_deg((%Player.global_position - global_position).angle_to(up_direction * 100)))
				#print("upside down")
				if angle > 0:
					left = -1
				else:
	...
	#
	### Detecting the ball in front to jump towards
	if raycast_collision != null and raycast_collision.get_name() == "Player":
		pressing_jump = 1
		landed = false
	else: 
		pressing_jump = 0
